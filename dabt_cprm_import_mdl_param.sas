		...
		*=============================== MODEL_MASTER & MM_REPORT_SPECIFICATION =============================;
			%if &m_mdl_in_tgt_flg eq &check_flag_true %then %do; 
				...
				%dabt_cprm_get_col_lst(	
										...
										m_exclued_col= level_sk project_sk model_sk model_id event_sk authorization_rule_id owned_by_user job_sk  lineage_status_sk, 
										m_col_lst=, 
			...
			%else %do ; 
				...
				*%dabt_make_work_area(dir=&project_path/&m_tgt_prj_sk, create_dir=/model/&m_next_model_sk, path=m_rel_path);
				...
				%dabt_cprm_get_col_lst(	
										...
										m_src_lib_nm= &m_cprm_src_apdm, 
										m_tgt_lib_nm= &m_apdm_lib, 
										m_exclued_col= level_sk project_sk model_sk model_id event_sk job_sk authorization_rule_id  lineage_status_sk deployment_status_sk, 
										m_col_lst=, 
										m_prim_col_nm=model_sk, 
										m_prim_col_val=&entity_sk,
				...
				%let job_sk_blank=.;
				%let deployment_status_sk_blank=.;
				proc sql noprint ; 
					insert into &m_apdm_lib..model_master 
					(&ins_cols_lst, model_sk, level_sk, project_sk, event_sk, model_id,job_sk,lineage_status_sk,deployment_status_sk) 
					select 	&ins_cols_lst, &m_next_model_sk.,
							...
							4 as lineage_status_sk,
							&deployment_status_sk_blank. as deployment_status_sk
	...
	/* CPRM CSB23670 - Extract SRC report specification sk */
		...
		quit;
				...
				proc sql noprint ; 	
					...
					select 	&ins_cols_lst, &m_next_rptspec_sk., &m_next_model_sk.  /* i18nOK:Line */
