%macro create_project_export_package(project_id_list=);
	...
	%let project_id_list = &project_id_list.;
	%let root_job_folder_uri =;
	%get_folder_uri(return_job_folder_uri=root_job_folder_uri,entity_type= PROJECT);
	...
	filename resp clear;
	%dabt_cprm_prj_export_wrapper (project_id_lst=&project_id_list., 
		export_ouput_folder_path=%str(/DevOps/Export/Projects/&m_job_sk./));
	filename imp_file filesrvc folderpath="/DevOps/Export/Projects/&m_job_sk./" 
	...
	data work.prj_imprt_specification_tmp;
		set work.prj_imprt_specification_tmp;

		IF upcase(var1)='PROJECT';
	run;

	proc sql noprint;
		create table work.prj_imprt_specification (entity_type_cd character(40) not 
	...
	proc sql noprint;
		insert into work.prj_imprt_specification
			...
			promote_flg, var7 as owner from work.prj_imprt_specification_tmp;
	quit;

	filename exp_file filesrvc folderpath="/DevOps/Export/Projects/&m_job_sk./" 
		filename="cprm_import_specification_file.csv" encoding='utf-8' debug=http;

	/* i18nOK:Line */
	proc export data=work.prj_imprt_specification outfile=exp_file DBMS=csv 
			replace label;
	run;
